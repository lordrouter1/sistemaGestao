...
function novaVar(root){
    ...
    let produto = root.attr('codigo'); 
    let temp = $('<div class="row p-2 stripedRow"></div>');
    r.forEach(item => {
        temp.append(`<div class="col"><select class="form-control" name="produto[${produto}][variacao][${cont}][variacoes][]" required><option selected disabled>${item['nome']}</option>${item['var'].map(v => `<option value="${item['_id']}:${v._id}">${v.nome}</option>`)}</select></div>`);
    });
    temp.append(`<div class="col"><input type="text" name="produto[${produto}][variacao][${cont}][preco]" placeholder="Preço" class="form-control" required></div>`);
    temp.append(`<div class="col"><input type="text" name="produto[${produto}][variacao][${cont}][minPreco]" placeholder="Preço mínimo" class="form-control"></div>`);
